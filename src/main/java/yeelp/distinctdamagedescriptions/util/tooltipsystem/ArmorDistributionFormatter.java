...
public class ArmorDistributionFormatter extends AbstractCapabilityTooltipFormatter<IArmorDistribution> {
	...
	protected Optional<List<String>> formatCapabilityFor(ItemStack stack, IArmorDistribution cap) {
		...
		switch(this.getNumberFormatter()) {
			case PLAIN:
				...
				return Optional.of(aMap.entrySet().stream().collect(LinkedList<String>::new, (l, e) -> l.add(makeOneArmorEffectivenessString(e.getValue().getArmor(), e.getValue().getToughness(), e.getKey())), LinkedList<String>::addAll));
			case PERCENT:
			default:
				return Optional.of(cap.getCategories().stream().collect(LinkedList<String>::new, (l, d) -> l.add(makeOneArmorEffectivenessString(cap.getWeight(d), d)), LinkedList<String>::addAll));
		}
	}
	...
	private String makeOneArmorEffectivenessString(float armor, float toughness, DDDDamageType type) {
		return String.format("   %s%s:(%s %s, %s %s)", this.getDamageFormatter().format(type), TextFormatting.GRAY.toString(), this.getNumberFormatter().format(armor), this.effectiveArmorSuffix.getFormattedText(), this.getNumberFormatter().format(toughness), this.effectiveToughnessSuffix.getFormattedText());
	}
}