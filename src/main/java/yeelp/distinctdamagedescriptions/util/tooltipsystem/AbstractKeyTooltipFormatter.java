...
public abstract class AbstractKeyTooltipFormatter<T> extends AbstractTooltipFormatter<T> {
	...
	/**
	 * Should the content of this formatter be shown? This does not need to indicate
	 * that no text is to be shown, simple text indicating "Hold KEY for info" is an
	 * expected use case. This merely indicates if additional content (the actual
	...
	/**
	 * Get the text for the key that needs to be held, if holding it actually
	 * changes tooltip content. Just calls {@code KeyTooltip.getKeyText()}
	 * 
	 * @return The String from the same call to the internal KeyTooltip, if the
	 *         "Never Hide Info" config option is set to false, otherwise an empty
	 *         String
